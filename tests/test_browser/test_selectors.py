...
from nocfo.fortnox.web.selectors import (
    _load_selectors,
...
class TestThreeStageResolution:
    ...
    def test_stage2_learned_match(self, mock_page, tmp_path):
        ...
        from nocfo.fortnox.web.learned import LearnedSelectors
        ...
        with patch("nocfo.fortnox.web.selectors._get_learned", return_value=store):
            result = find(mock_page, "login.bankid_tab", timeout=5000)
    ...
    def test_stage3_vision_fallback(self, mock_page, tmp_path):
        ...
        with (
            patch("nocfo.fortnox.web.selectors._get_learned", return_value=store),
            patch("nocfo.fortnox.web.vision.find_element", return_value=mock_handle) as mock_vision,
        ):
            result = find(
