...
from nocfo.browser.chrome import (
    DEFAULT_CDP_PORT,
    find_chrome,
    is_cdp_reachable,
)
...
class TestFindChrome:
    def test_find_chrome_on_macos(self):
        ...
        with patch("nocfo.browser.chrome.Path") as mock_path:
            instance = MagicMock()
    ...
    def test_find_chrome_not_found(self):
        ...
        with patch("nocfo.browser.chrome.Path") as mock_path:
            ...
            mock_path.return_value = instance
            with patch("nocfo.browser.chrome.shutil.which", return_value=None):
                try:
                    find_chrome()
...
class TestCdpReachable:
    def test_reachable_when_connected(self):
        ...
        with patch("nocfo.browser.chrome.socket.create_connection") as mock_conn:
    ...
    def test_not_reachable_when_refused(self):
        ...
        with patch(
            "nocfo.browser.chrome.socket.create_connection",
