...
from fortnox.web.session import load_session, save_session
...
class BrowserAPIHandler(BaseHTTPRequestHandler):
    ...
    def _dispatch(self, method: str) -> None:
        ...
        if path == "/health" and method == "GET":
            from fortnox.browser.tokens import cleanup_expired_tokens
            cleanup_expired_tokens()
            return self._handle_health()

        # Token-authenticated endpoints (no bearer auth)
        if path == "/auth/live" and method == "GET":
            from fortnox.browser.tokens import validate_token_for_stream
        ...
        if path == "/auth/stream" and method == "GET":
            from fortnox.browser.tokens import validate_token_for_stream, get_token_context
        ...
        if path.startswith("/operation/") and method == "GET":
            from fortnox.browser.operations_state import get_operation
            parts = path.split("/")
            if len(parts) >= 3:
                op_id = parts[2]
    ...
    def _handle_sse_stream(self, op_id: str, token: str, is_mobile: bool = False) -> None:
        ...
        from fortnox.browser.tokens import get_token_context
        ...
        if op.get("status") == "awaiting_user":
            from fortnox.web.handlers import trigger_bankid_flow
