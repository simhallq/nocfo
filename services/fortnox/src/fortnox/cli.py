...
def status(health: bool) -> None:
    ...
    if is_auth:
        ...
        click.echo("Authenticated: No - run 'fortnox auth setup'")
        return
...
def close_run(month: str, headed: bool, browser_api: bool) -> None:
    ...
    if browser_api:
        ...
        async def _close():
            from fortnox.web_agent.browser import BrowserManager
            from fortnox.web_agent.tasks.period_closing import run_period_closing
...
def schedule_start() -> None:
    ...
    from fortnox.scheduler.runner import Scheduler
...
def report(report_type: str, period: str, headed: bool, browser_api: bool) -> None:
    ...
    if browser_api:
        ...
        async def _download():
            from fortnox.web_agent.browser import BrowserManager
            from fortnox.web_agent.tasks.reports import run_report_download
...
def browser_start(headless: bool) -> None:
    ...
    from fortnox.browser.chrome import is_cdp_reachable, launch_chrome
    from fortnox.browser.server import run_server
    from fortnox.config import get_settings
...
def record_replay(
    ...
    from fortnox.config import get_settings
    from fortnox.recorder.models import Workflow
...
def svd_invoice(speed: float, cdp_url: str) -> None:
    ...
    if not yaml_path.exists():
        click.echo(
            ...
            "Record it first: fortnox record start get_svd_invoice "
...
def record_enhance(name: str) -> None:
    ...
    from fortnox.config import get_settings
    from fortnox.recorder.enhancer import enhance_workflow
    from fortnox.recorder.models import Workflow
