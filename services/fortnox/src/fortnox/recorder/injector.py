...
RECORDER_JS_MAIN = """
(() => {
    if (window.__fortnox_recorder_active) return;
    window.__fortnox_recorder_active = true;

    // Recording indicator (main frame only)
    if (window === window.top) {
        const dot = document.createElement('div');
        dot.id = '__fortnox_record_indicator';
        dot.style.cssText = [
            ...
            'animation:__fortnox_pulse 1.5s infinite',
        ].join(';');
        const style = document.createElement('style');
        style.textContent = '@keyframes __fortnox_pulse{0%,100%{opacity:1}50%{opacity:0.4}}';
        document.head.appendChild(style);
        document.body.appendChild(dot);

        // Listen for events relayed from iframes via postMessage
        window.addEventListener('message', (e) => {
            if (!e.data || !e.data.__fortnox_event) return;
            if (typeof window.__fortnox_record_event !== 'function') return;
            window.__fortnox_record_event(e.data.__fortnox_event);
        });
    }
    ...
    function sendEvent(payloadJson) {
        if (window === window.top && typeof window.__fortnox_record_event === 'function') {
            window.__fortnox_record_event(payloadJson);
        } else if (window !== window.top) {
            try {
                window.top.postMessage({ __fortnox_event: payloadJson }, '*');
            } catch(e) {}
        }
    }
...
def inject_recorder(page: Page, callback) -> None:
    ...
    """
    page.expose_function("__fortnox_record_event", callback)
